...
extension WhoopStore {
    ...
    /// Pending (synced=0) row counts across ALL decoded tables for a single device, in ONE
    /// round-trip. Used by the developer diagnostics + the Uploader's "what's about to ship"
    /// log line so an empty /v1/ingest-decoded payload is immediately attributable to a
    /// specific stream (collected-but-no-pending vs pending-but-not-sent vs received-but-not-
    /// marked-synced). Table names are hardcoded — no injection.
    public func unsyncedCountsAll(deviceId: String) async throws -> [String: Int] {
        try syncRead { db -> [String: Int] in
            ...
            return [
                ...
                "skin_temp":try count("skinTempSample"),
