...
public struct CachedSleepSession: Equatable, Codable {
    ...
    public init(startTs: Int, endTs: Int, efficiency: Double?, restingHr: Int?,
                ...
                source: MetricValueSource = .unknown,
                confidence: Double? = nil,
                status: MetricAvailabilityStatus = .ready,
                unavailableReason: String? = nil,
                recomputedAt: Int? = nil) {
        ...
        self.status = status; self.unavailableReason = unavailableReason
                ...
                try db.execute(sql: """
                    INSERT INTO sleepSession
                        (deviceId, startTs, endTs, efficiency, restingHr, avgHrv, stagesJSON,
                         source, confidence, status, unavailableReason, recomputedAt)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ...
                    """, arguments: [deviceId, s.startTs, s.endTs, s.efficiency,
                                     ...
                                     s.source.rawValue, s.confidence, s.status.rawValue,
                                     s.unavailableReason, s.recomputedAt])
                n += db.changesCount
            }
            return n
        }
                ...
                try db.execute(sql: """
                    INSERT INTO dailyMetric
                        (deviceId, day, totalSleepMin, efficiency, deepMin, remMin, lightMin,
                         ...
                         spo2Pct, skinTempDevC, respRateBpm, source, confidence, status,
                         unavailableReason, recomputedAt)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ...
                    """, arguments: [deviceId, d.day, d.totalSleepMin, d.efficiency, d.deepMin,
                                     ...
                                     d.source.rawValue, d.confidence, d.status.rawValue,
                                     d.unavailableReason, d.recomputedAt])
                n += db.changesCount
            }
            return n
        }
    ...
    public func sleepSessions(deviceId: String, from: Int, to: Int, limit: Int) async throws -> [CachedSleepSession] {
        try syncRead { db in
            try Row.fetchAll(db, sql: """
                ...
                .map {
                    let source = MetricValueSource(rawValue: $0["source"] ?? "") ?? .unknown
                    let status = MetricAvailabilityStatus(rawValue: $0["status"] ?? "") ?? .ready
                    return CachedSleepSession(startTs: $0["startTs"], endTs: $0["endTs"],
                                              efficiency: $0["efficiency"], restingHr: $0["restingHr"],
                                              avgHrv: $0["avgHrv"], stagesJSON: $0["stagesJSON"],
                                              source: source, confidence: $0["confidence"],
                                              status: status, unavailableReason: $0["unavailableReason"],
                                              recomputedAt: $0["recomputedAt"])
                }
        }
    }
        ...
        try syncRead { db in
            try Row.fetchAll(db, sql: """
                SELECT day, totalSleepMin, efficiency, deepMin, remMin, lightMin, disturbances,
                       ...
                       spo2Pct, skinTempDevC, respRateBpm, source, confidence, status,
                       unavailableReason, recomputedAt FROM dailyMetric
                ...
                .map {
                    let source = MetricValueSource(rawValue: $0["source"] ?? "") ?? .unknown
                    let status = MetricAvailabilityStatus(rawValue: $0["status"] ?? "") ?? .ready
                    return DailyMetric(day: $0["day"], totalSleepMin: $0["totalSleepMin"],
                                       ...
                                       respRateBpm: $0["respRateBpm"], source: source,
                                       confidence: $0["confidence"], status: status,
