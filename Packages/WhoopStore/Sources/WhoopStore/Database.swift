...
extension WhoopStore {
    ...
    static func makeMigrator() -> DatabaseMigrator {
        ...
        migrator.registerMigration("v8") { db in
            // Local-owned metric metadata. The legacy server-cache rows remain readable and simply
            // default to "unknown/ready" until on-device recompute overwrites them.
            try db.alter(table: "sleepSession") { t in
                t.add(column: "source", .text).notNull().defaults(to: "unknown")
                t.add(column: "confidence", .double)
                t.add(column: "status", .text).notNull().defaults(to: "ready")
                t.add(column: "unavailableReason", .text)
                t.add(column: "recomputedAt", .integer)
            }
            try db.alter(table: "dailyMetric") { t in
                t.add(column: "source", .text).notNull().defaults(to: "unknown")
                t.add(column: "confidence", .double)
        ...
        migrator.registerMigration("v9") { db in
            try db.create(table: "recoveryMetric") { t in
                t.column("deviceId", .text).notNull()
                t.column("day", .text).notNull()
                ...
                t.column("baselineProgress", .integer).notNull().defaults(to: 0)
                t.column("explanation", .text)
                t.column("recomputedAt", .integer)
                t.primaryKey(["deviceId", "day"])
            }
            try db.create(table: "strainMetric") { t in
                t.column("deviceId", .text).notNull()
                t.column("day", .text).notNull()
                t.column("score", .double)
                t.column("status", .text).notNull()
            ...
            try db.create(table: "metricBaseline") { t in
                t.column("deviceId", .text).notNull()
                ...
                t.column("recomputedAt", .integer)
                t.primaryKey(["deviceId", "metric", "day"])
            }
            try db.create(table: "metricDiagnostics") { t in
                t.column("deviceId", .text).notNull()
                t.column("key", .text).notNull()
                t.column("value", .text).notNull()
                t.column("updatedAt", .integer)
                t.primaryKey(["deviceId", "key"])
            }
            try db.create(table: "workoutSession") { t in
                ...
                t.column("status", .text).notNull()
                t.column("heartRateSamplesJSON", .text).notNull()
                t.column("accelerometerSamplesJSON", .text).notNull()
                t.column("gyroscopeSamplesJSON", .text).notNull()
                t.column("averageHeartRate", .double)
                t.column("maxHeartRate", .double)
                t.column("cardioLoad", .double).notNull().defaults(to: 0)
                t.column("muscularLoad", .double).notNull().defaults(to: 0)
                t.column("dailyStressLoad", .double).notNull().defaults(to: 0)
                t.column("totalLoad", .double).notNull().defaults(to: 0)
                t.column("strainScore", .double).notNull().defaults(to: 0)
                t.column("userIntensity", .integer)
                t.column("muscularConfidence", .double).notNull().defaults(to: 0)
                t.column("recoveryAdjustment", .double).notNull().defaults(to: 1)
                t.column("baselineStrain", .double)
                t.column("zoneBreakdownJSON", .text).notNull()
                t.column("updatedAt", .integer)
                t.primaryKey(["deviceId", "sessionId"])
            }
        }
        migrator.registerMigration("v10") { db in
            // Indexes used by local-only reads and diagnostics.
            try db.create(index: "idx_rawBatch_capturedAt", on: "rawBatch", columns: ["capturedAt"])
            try db.create(index: "idx_workoutSession_startTime", on: "workoutSession", columns: ["deviceId", "startTime"])
        }
        return migrator
    }
}
