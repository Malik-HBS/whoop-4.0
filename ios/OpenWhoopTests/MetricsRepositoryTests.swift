...
final class MetricsRepositoryTests: XCTestCase {
    ...
    private func seedDaily(_ store: WhoopStore) async throws -> [DailyMetric] {
        ...
        let now = Date()
        let earlier = Calendar(identifier: .gregorian).date(byAdding: .day, value: -2, to: now) ?? now
        let later = Calendar(identifier: .gregorian).date(byAdding: .day, value: -1, to: now) ?? now
        let days = [
            DailyMetric(day: formatter.string(from: earlier), totalSleepMin: 400, efficiency: 0.85,
                        deepMin: 80, remMin: 100, lightMin: 220, disturbances: 2,
                        restingHr: 55, avgHrv: 58, recovery: 0.62, strain: 10, exerciseCount: 1),
            DailyMetric(day: formatter.string(from: later), totalSleepMin: 430, efficiency: 0.90,
