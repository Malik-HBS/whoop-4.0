...
final class CollectorTests: XCTestCase {
    ...
    func testType43IMUPersistsGravityRowsWhenRawCaptureIsOff() async throws {
        let store = try await makeStore()
        let c = Collector(store: store, deviceId: "my-whoop",
                          policy: .init(maxFrames: 1, maxInterval: 60))
        c.clockRef = ClockRef(device: 31_538_447, wall: 1_716_400_000)
        c.ingest(makeType43IMUFrame())
        await c.flush()

        let gravity = try await store.gravitySamples(deviceId: "my-whoop", from: 0, to: Int.max, limit: 10)
        let stats = try await store.storageStats()
        XCTAssertFalse(gravity.isEmpty, "IMU frame should derive and persist at least one gravity row")
        XCTAssertEqual(stats.rawBatches, 0, "raw capture remains off by default")
    }

    func testType43OpticalDoesNotPersistGravityRows() async throws {
        let store = try await makeStore()
        let c = Collector(store: store, deviceId: "my-whoop",
                          policy: .init(maxFrames: 1, maxInterval: 60))
        c.clockRef = ClockRef(device: 31_538_448, wall: 1_716_400_000)
        c.ingest(makeType43OpticalFrame())
        await c.flush()

        let gravity = try await store.gravitySamples(deviceId: "my-whoop", from: 0, to: Int.max, limit: 10)
        XCTAssertTrue(gravity.isEmpty, "optical type-43 frames must not create gravity rows")
    }
}
