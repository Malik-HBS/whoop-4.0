...
final class FrameRouterTests: XCTestCase {
    ...
    @MainActor func testRealtimeCallbackUsesSameVisibleHRValue() {
        let state = LiveState()
        let router = FrameRouter(state: state)
        var callbackHR: Int?
        var callbackRR: [Int] = []
        router.onRealtimeHeartRate = { hr, rr in
            callbackHR = hr
            callbackRR = rr
        }
        ...
        XCTAssertEqual(callbackHR, state.heartRate, "callback must receive the exact HR shown in the UI")
        XCTAssertEqual(callbackRR, state.rr, "callback must receive the same RR intervals surfaced in the UI")
    }
