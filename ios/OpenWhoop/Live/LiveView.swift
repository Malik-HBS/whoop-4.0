...
public struct LiveView: View {
    ...
    @EnvironmentObject var serverDiagnostics: ServerDiagnostics
    @EnvironmentObject var bleDiagnostics: BLEDiagnostics
    @Environment(\.scenePhase) private var scenePhase
    public init() {}
    public var body: some View {
        LiveContentView(state: model.state, model: model, metrics: metrics,
                        serverDiagnostics: serverDiagnostics, bleDiagnostics: bleDiagnostics)
            ...
            .task {
                while !Task.isCancelled {
                    ...
                    model.refreshLocalStoreSnapshot()
...
private struct LiveContentView: View {
    ...
    @ObservedObject var serverDiagnostics: ServerDiagnostics
    @ObservedObject var bleDiagnostics: BLEDiagnostics

    @State private var showingSettings = false
    @State private var showingServerDiagnostics = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingServerDiagnostics) {
    ...
    private var serverDiagnosticsRow: some View {
        Button {
            showingServerDiagnostics = true
        } label: {
            HStack(spacing: WH.Spacing.sm) {
                Image(systemName: "network")
                    .font(.system(size: 18, weight: .medium))
                    .foregroundStyle(serverDiagnostics.isConfigured ? WH.Color.recoveryGreen : WH.Color.recoveryYellow)
                    .frame(width: 32, height: 32)
                    .background((serverDiagnostics.isConfigured ? WH.Color.recoveryGreen : WH.Color.recoveryYellow).opacity(0.15),
                                in: RoundedRectangle(cornerRadius: WH.Radius.small, style: .continuous))
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(serverStatusSubtitle)
                        .font(WH.Font.caption)
                        .foregroundStyle(serverDiagnostics.isConfigured ? WH.Color.textSecondary : WH.Color.recoveryYellow)
                }
    ...
    private var serverStatusSubtitle: String {
        if !serverDiagnostics.isConfigured {
        ...
        return "Base URL, API key, upload health, and optional sync status"
    }
    ...
    private var researchSection: some View {
        consoleCard {
            VStack(alignment: .leading, spacing: WH.Spacing.sm) {
                ...
                consoleButton("Run Gravity Test 60s",
                              icon: "sensor.tag.radiowaves.forward",
                              accent: WH.Color.recoveryYellow,
                              prominent: false) {
                    model.runMotionGravityTest(seconds: 60)
                }

                Text("Starts the type-43 IMU stream for 60 seconds, derives gravity rows locally, and keeps raw storage optional.")
                    .font(WH.Font.caption)
                    .foregroundStyle(WH.Color.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
                ...
                sectionHeader("IMU/Gravity Diagnostics")

                consoleButton("Run Legacy Raw Accel 30s",
                              icon: "arrow.clockwise",
                              accent: WH.Color.teal, prominent: false) {
                    model.runLegacyRawAccelTest(seconds: 30)
                }
                Text("Runs the legacy captureRawAccel sequence and compares frame43 counts with the new gravity stream.")
                    .font(WH.Font.caption)
                    .foregroundStyle(WH.Color.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)

                consoleButton("Try IMU Sequence A",
                              icon: "1.circle",
                              accent: WH.Color.strainBlue, prominent: false) {
                    model.tryIMUSequence(.sequenceA)
                }
                consoleButton("Try IMU Sequence B",
                              icon: "2.circle",
                              accent: WH.Color.strainBlue, prominent: false) {
                    model.tryIMUSequence(.sequenceB)
                }
                consoleButton("Try IMU Sequence C",
                              icon: "3.circle",
                              accent: WH.Color.strainBlue, prominent: false) {
                    model.tryIMUSequence(.sequenceC)
                }
                consoleButton("Try IMU Sequence D",
                              icon: "4.circle",
                              accent: WH.Color.strainBlue, prominent: false) {
                    model.tryIMUSequence(.sequenceD)
                }
                consoleButton("Try IMU Sequence E",
                              icon: "5.circle",
                              accent: WH.Color.strainBlue, prominent: false) {
                    model.tryIMUSequence(.sequenceE)
                }
                consoleButton("Try IMU Sequence F",
                              icon: "6.circle",
                              accent: WH.Color.strainBlue, prominent: false) {
                    model.tryIMUSequence(.sequenceF)
                }
                consoleButton("Try IMU Sequence G",
                              icon: "7.circle",
                              accent: WH.Color.strainBlue, prominent: false) {
                    model.tryIMUSequence(.sequenceG)
                }
                consoleButton("Try IMU Sequence H",
                              icon: "8.circle",
                              accent: WH.Color.strainBlue, prominent: false) {
                    model.tryIMUSequence(.sequenceH)
                }

                Text("Each sequence tests a different START/TOGGLE order with/without delays. Results shown in diagnostics.")
                    .font(WH.Font.caption)
                    .foregroundStyle(WH.Color.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)

                Toggle("Keep IMU Stream On While Open",
                       isOn: Binding(
                           get: { bleDiagnostics.keepIMUStreamOnWhileOpen },
                           set: { bleDiagnostics.setKeepIMUStreamOnWhileOpen($0) }
                       ))
                    .tint(WH.Color.strainBlue)
                    .foregroundStyle(WH.Color.textPrimary)

                Text("When enabled, starts IMU stream and keeps it running until you leave this screen or toggle off.")
                    .font(WH.Font.caption)
                    .foregroundStyle(WH.Color.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
