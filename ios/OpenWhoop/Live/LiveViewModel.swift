...
public final class LiveViewModel: ObservableObject {
    ...
    init(deviceId: String = "my-whoop", stepCountingService: StepCountingService? = nil) {
        let s = LiveState()
        self.state = s
        self.ble = BLEManager(state: s, deviceId: deviceId, stepCountingService: stepCountingService)
    ...
    /// Bounded developer gravity test: enables the type-43 IMU stream long enough to derive and
    /// persist local gravity rows without requiring raw frame storage.
    public func runMotionGravityTest(seconds: TimeInterval = 60) { ble.runMotionGravityTest(seconds: seconds) }

    /// Run the legacy captureRawAccel sequence for comparison.
    public func runLegacyRawAccelTest(seconds: TimeInterval = 30) { ble.runLegacyRawAccelTest(seconds: seconds) }

    /// Try a specific IMU command sequence for testing.
    public func tryIMUSequence(_ sequence: IMUSequence) { ble.tryIMUSequence(sequence) }
    ...
    /// Refresh the per-decoded-stream count + latest-ts snapshot into BLEDiagnostics
    /// (the Developer Diagnostics card reads from there). Cheap: one SQLite read pass.
    public func refreshLocalStoreSnapshot() {
        Task { @MainActor in await ble.refreshLocalStoreSnapshot() }
    }
}
