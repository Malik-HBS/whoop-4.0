...
struct SleepView: View {
    ...
    @State private var sleepScore: SleepScoreResult?
    ...
    private func loadData() async {
        ...
        sleepScore = await metrics.sleepScore()
    ...
    private var scrollContent: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: WH.Spacing.lg) {
                ...
                // 2. Headline — efficiency hero + total duration
                headlineSection
    ...
    private var sleepScoreCard: some View {
        ...
        return VStack(alignment: .leading, spacing: WH.Spacing.md) {
            HStack(alignment: .top, spacing: WH.Spacing.md) {
                VStack(alignment: .leading, spacing: WH.Spacing.xs) {
                    ...
                    case .buildingBaseline?:
                        Text("Building baseline")
                            .font(WH.Font.metricLarge(size: 30))
                ...
                if let result, result.status == .buildingBaseline {
                    VStack(alignment: .trailing, spacing: WH.Spacing.xs) {
                        Text("\(result.baselineProgress)/\(result.requiredBaselineNights)")
                            .font(WH.Font.metricMedium(size: 24))
                            .foregroundStyle(WH.Color.sleepPurple)
                            .monospacedDigit()
                        Text("nights collected")
                            .font(WH.Font.caption)
                            .foregroundStyle(WH.Color.textSecondary)
                    }
                }
            ...
            if let result {
                ...
                if let components = result.components {
                    VStack(spacing: WH.Spacing.xs) {
                        componentRow("Need", value: components.sleepNeedCompletion, max: 35)
                        componentRow("Efficiency", value: components.sleepEfficiency, max: 20)
                        componentRow("Awake", value: components.disturbances, max: 15)
                        componentRow("Recovery", value: components.recovery, max: 15)
                        componentRow("Consistency", value: components.consistency, max: 10)
                        componentRow("REM/Deep", value: components.architecture, max: 5)
                    }
                }
            }
        }
    ...
    private func componentRow(_ label: String, value: Double, max maximum: Double) -> some View {
        HStack(spacing: WH.Spacing.sm) {
            Text(label)
                .font(WH.Font.caption)
                .foregroundStyle(WH.Color.textSecondary)
            ...
            GeometryReader { proxy in
                ZStack(alignment: .leading) {
                    ...
                    Capsule()
                        ...
                        .frame(width: proxy.size.width * min(1, Swift.max(0, value / maximum)))
                }
            }
            .frame(height: 6)
            Text("\(Int(value.rounded()))/\(Int(maximum))")
                .font(WH.Font.caption)
                .foregroundStyle(WH.Color.textPrimary)
                .monospacedDigit()
    ...
    private func subtitle(for result: SleepScoreResult) -> String {
        switch result.status {
        case .buildingBaseline:
            return "\(result.baselineProgress)/\(result.requiredBaselineNights) nights collected"
        case .ready:
    ...
    private func scoreColor(_ score: Int?) -> Color {
        guard let score else { return WH.Color.textSecondary }
        switch score {
        case 80...100: return WH.Color.recoveryGreen
        case 70...79: return WH.Color.recoveryYellow
