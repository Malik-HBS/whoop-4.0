...
public final class FrameRouter {
    public enum HandleResult: Equatable {
        case ignoredParse
        case ignoredCRC
        case realtimeDataDecoded(hr: Int, rr: [Int])
        case realtimeDataMissingHeartRate
    ...
    /// Called immediately after the UI-visible live HR is updated from a REALTIME_DATA frame.
    /// BLEManager uses this to persist the exact same value into the local store.
    var onRealtimeHeartRate: ((_ hr: Int, _ rr: [Int]) -> Void)?
    ...
    @discardableResult
    public func handle(frame: [UInt8]) -> HandleResult {
        let parsed = parseFrame(frame)
        guard parsed.ok else { return .ignoredParse }
        // Reject frames that failed their checksum — never let bad bytes drive state.
        if parsed.crcOK == false { return .ignoredCRC }
        ...
        case "REALTIME_DATA":
            ...
            if let hr = parsed.parsed["heart_rate"]?.intValue {
                onRealtimeHeartRate?(hr, rr)
                return .realtimeDataDecoded(hr: hr, rr: rr)
            }
            return .realtimeDataMissingHeartRate
