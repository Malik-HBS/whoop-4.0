...
public final class BLEManager: NSObject, ObservableObject {
    ...
    /// Separate from research raw capture: this controls a bounded developer test that turns
    /// on the type-43 IMU stream so derived gravity rows can be persisted locally.
    private var motionGravityTestInFlight = false
    private var motionGravityStopOnExpiry = false
    private var motionGravityStreamActive = false
    ...
    init(state: LiveState, deviceId: String = "my-whoop", stepCountingService: StepCountingService? = nil) {
        self.state = state
        ...
        router.onRealtimeHeartRate = { [weak self] hr, rr in
            self?.persistRealtimeUIHeartRate(hr: hr, rrIntervalsMs: rr)
        }
    }
    ...
    func bootstrapStore() async {
        ...
        BLEDiagnostics.shared.setRawCaptureEnabled(enableRawCapture)
    ...
    init(state: LiveState, deviceId: String = "my-whoop", collector: Collector?, stepCountingService: StepCountingService? = nil) {
        self.state = state
        self.deviceId = deviceId
        self.router = FrameRouter(state: state)
    ...
    /// Refresh the per-decoded-stream count + latest-ts snapshot into the BLEDiagnostics store.
    /// Called from the LiveView polling task every 5 s so the Developer Diagnostics card stays
    /// current. No-op without a concrete store.
    public func refreshLocalStoreSnapshot() async {
        guard let collector else { return }
        guard let snap = await collector.localStoreSnapshot() else { return }
        BLEDiagnostics.shared.setLocalSnapshot(snap)
        // Also refresh raw capture stats from the same concrete store so the card stays in sync.
        if let stats = await collector.storageStats() {
            BLEDiagnostics.shared.setRawLocalStats(
                batches: stats.rawBatches,
                bytes: stats.rawBytes,
                latestAt: await collector.latestRawBatchCapturedAt()
            )
        }
    }
    ...
    public func startMotionGravityStream() {
        guard state.connected else {
            log("Motion/gravity stream: ignored — not connected")
            return
        }
        guard state.bonded || didBond else {
            log("Motion/gravity stream: ignored — link not bonded yet")
            return
        }
        guard connectHandshakeDone else {
            log("Motion/gravity stream: ignored — connect handshake not done")
            return
        }
        guard cmdCharacteristic != nil else {
            log("Motion/gravity stream: ignored — command characteristic unavailable")
            return
        }

        motionGravityStreamActive = true
        BLEDiagnostics.shared.setMotionGravityStreamActive(true)
        sendMotionGravityCommand(.sendR10R11Realtime, payload: [0x01])
        sendMotionGravityCommand(.startRawData, payload: [0x01])
        sendMotionGravityCommand(.toggleIMUMode, payload: [0x01])
        log("Motion/gravity stream: started (R10R11 realtime ON + START_RAW_DATA + TOGGLE_IMU)")
    }

    public func stopMotionGravityStream() {
        guard motionGravityStreamActive else { return }
        sendMotionGravityCommand(.sendR10R11Realtime, payload: [0x00])
        sendMotionGravityCommand(.stopRawData, payload: [0x01])
        motionGravityStreamActive = false
        motionGravityStopOnExpiry = false
        BLEDiagnostics.shared.setMotionGravityStreamActive(false)
        log("Motion/gravity stream: stopped")
    }

    public func runLegacyRawAccelTest(seconds: TimeInterval = 30) {
        captureRawAccel(seconds: seconds)
    }

    public func tryIMUSequence(_ sequence: IMUSequence) {
        log("IMU sequence: \(sequence.label)")
        let initialFrame43Count = BLEDiagnostics.shared.frame43Count
        switch sequence {
        case .sequenceA:
            sendMotionGravityCommand(.startRawData, payload: [0x01])
        case .sequenceB:
        ...
        case .sequenceD:
            ...
            sendMotionGravityCommand(.startRawData, payload: [0x01])
        case .sequenceE:
            sendMotionGravityCommand(.startRawData, payload: [0x00])
            sendMotionGravityCommand(.toggleIMUMode, payload: [0x01])
        case .sequenceF:
            sendMotionGravityCommand(.startRawData, payload: [0x01])
            DispatchQueue.main.asyncAfter(deadline: .now() + 2) { [weak self] in
                self?.sendMotionGravityCommand(.toggleIMUMode, payload: [0x01])
            }
        case .sequenceG:
            sendMotionGravityCommand(.toggleIMUMode, payload: [0x01])
            DispatchQueue.main.asyncAfter(deadline: .now() + 2) { [weak self] in
                self?.sendMotionGravityCommand(.startRawData, payload: [0x01])
            }
        case .sequenceH:
            sendMotionGravityCommand(.sendR10R11Realtime, payload: [0x01])
            sendMotionGravityCommand(.startRawData, payload: [0x01])
        ...
        log("IMU sequence \(sequence.label): sent commands (frame43 was \(initialFrame43Count))")
    }

    public func runMotionGravityTest(seconds: TimeInterval = 60) {
        guard !motionGravityTestInFlight else {
            log("Motion/gravity test: already in flight — ignoring")
            return
        }
        motionGravityTestInFlight = true
        motionGravityStopOnExpiry = true
        let secs = RawCaptureWindow.clamp(seconds)
        startMotionGravityStream()
        guard motionGravityStreamActive else {
            motionGravityTestInFlight = false
            motionGravityStopOnExpiry = false
            return
        }
        log("Motion/gravity test: running for \(secs)s")
        DispatchQueue.main.asyncAfter(deadline: .now() + secs) { [weak self] in
            guard let self else { return }
            if self.motionGravityStopOnExpiry {
                self.stopMotionGravityStream()
            }
            self.motionGravityTestInFlight = false
            self.log("Motion/gravity test: completed")
        }
    }
    ...
    private func sendMotionGravityCommand(_ command: WhoopCommand,
                                          payload: [UInt8],
                                          writeType: CBCharacteristicWriteType = .withoutResponse) {
        BLEDiagnostics.shared.recordMotionGravityCommand(command, payload: payload)
        send(command, payload: payload, writeType: writeType)
    }
    ...
    private func beginBackfill() {
        ...
        backfiller.begin()
        backfilling = true
        BLEDiagnostics.shared.recordBackfillRequested(payload: [0x00])
        BLEDiagnostics.shared.recordBackfillStart()
    ...
    private func afterBackfillIngest() {
        ...
        BLEDiagnostics.shared.recordBackfillComplete()
    ...
    private func exitBackfilling(reason: String) {
        ...
        case "HISTORY_COMPLETE":
            BLEDiagnostics.shared.recordBackfillComplete()
        case "timeout":
            BLEDiagnostics.shared.recordBackfillTimeout()
        default:
            BLEDiagnostics.shared.recordBackfillReason(reason)
        }
    ...
    /// Updates LiveState (UI) AND persists into the local store via the Collector, so the
    /// server eventually sees the live HR + R-R (this is the central Phase 2 fix — pre-fix the
    /// standard 0x2A37 path only touched UI state, which is why live HR was visible but the
    /// server summary said hr=0).
    private func parseStandardHR(_ data: [UInt8]) {
        BLEDiagnostics.shared.recordStandardHRNotification()
        ...
        // Persist into the local store. Pre-collector (store not bootstrapped yet) the call
        // is a no-op — the next notification will persist as soon as the store opens. The
        // Collector's own dedupe (bpm + same-second ts) handles the high-frequency case.
        let now = Date()
        Task { @MainActor in
            let result = await collector?.ingestLiveHeartRate(
                hr: m.hr,
                rrIntervalsMs: m.rr,
                wallTime: now,
                source: .standardBLE2A37
            )
            BLEDiagnostics.shared.recordLiveUIHeartRate(value: m.hr, saved: result?.insertedHR)
            if result?.hrDeduped == true {
                BLEDiagnostics.shared.recordStandardHRSkippedDedupe()
            }
            BLEDiagnostics.shared.recordStandardHRSaved(
                result?.insertedHR == true,
                rr: result?.insertedRRCount ?? 0
            )
            if result?.insertedHR == true || (result?.insertedRRCount ?? 0) > 0 {
                NotificationCenter.default.post(name: .didPersistLocalBiometrics, object: nil)
            }
        }
    }

    /// Persist the exact live-HR value that the UI just displayed from the REALTIME_DATA path.
    /// This intentionally bypasses clock-correlation and frame re-parsing: the UI already proved
    /// the HR value is decoded and present, so we persist that same value immediately.
    private func persistRealtimeUIHeartRate(hr: Int, rrIntervalsMs: [Int]) {
        BLEDiagnostics.shared.recordFrame40Decoded(hr: hr, rrCount: rrIntervalsMs.count)
        BLEDiagnostics.shared.recordLiveUIHeartRate(value: hr, saved: nil)
        guard let collector else {
            BLEDiagnostics.shared.recordFrame40PersistResult(.failed(
                attemptedHR: hr > 0,
                attemptedRRCount: rrIntervalsMs.count,
                error: "collector not ready"
            ))
            BLEDiagnostics.shared.recordLiveUIHeartRate(value: hr, saved: false)
            return
        }
        let now = Date()
        Task { @MainActor in
            let result = await collector.ingestLiveHeartRate(
                ...
                wallTime: now,
                source: .whoopRealtime40
            )
            BLEDiagnostics.shared.recordFrame40PersistResult(result)
            BLEDiagnostics.shared.recordLiveUIHeartRate(value: hr, saved: result.insertedHR)
            if result.insertedHR || result.insertedRRCount > 0 {
                NotificationCenter.default.post(name: .didPersistLocalBiometrics, object: nil)
            }
        }
    }
...
extension BLEManager: CBCentralManagerDelegate {
    ...
    public func centralManager(_ central: CBCentralManager,
        ...
        motionGravityStreamActive = false
        BLEDiagnostics.shared.setMotionGravityStreamActive(false)
        BLEDiagnostics.shared.setDataNotifyActive(false)
...
extension BLEManager: CBPeripheralDelegate {
    ...
    public func peripheral(_ peripheral: CBPeripheral,
        ...
        for c in chars {
            ...
            case BLEManager.cmdNotifyChar,
                ...
                if c.uuid == BLEManager.dataNotifyChar {
                    BLEDiagnostics.shared.setDataNotifyActive(true)
                }
    ...
    public func peripheral(_ peripheral: CBPeripheral,
        ...
        if connectHandshakeDone, backfillStarted {
            BLEDiagnostics.shared.recordBackfillAckSeen()
        }
        if !didBond {
            didBond = true
            state.bonded = true
    ...
    static func dataRangeNewestUnix(from frame: [UInt8]) -> Int? {
        dataRangeBounds(from: frame)?.newest
    }

    static func dataRangeBounds(from frame: [UInt8]) -> (oldest: Int?, newest: Int?)? {
    ...
    public func peripheral(_ peripheral: CBPeripheral,
        ...
        case BLEManager.dataNotifyChar,
            ...
            for frame in reassembler.feed(bytes) {
                if characteristic.uuid == BLEManager.cmdNotifyChar {
                    BLEDiagnostics.shared.recordMotionGravityCommandResponse(frame)
                }
                // Per-frame type counter (developer diagnostics). frame[4] is the on-wire
                // type byte; the switch is bounded to 40/43/47/48/49/50 and silent otherwise.
                if frame.count > 4 {
                    BLEDiagnostics.shared.recordFrame(frame[4])
                }
                let frameType = frame.count > 4 ? frame[4] : nil
                if frameType == 43, let info = type43PacketInfo(from: frame) {
                    BLEDiagnostics.shared.recordType43PacketLength(info.dataLength)
                    switch info.kind {
                    case .imu:
                        let hasClock = clockRef != nil
                        BLEDiagnostics.shared.recordType43IMUDecodeAttempt()
                        if hasClock {
                            let samples = extractAccelerometerSamples(from: frame,
                                                                      deviceClockRef: clockRef?.device ?? 0,
                                                                      wallClockRef: clockRef?.wall ?? 0)
                            if samples.isEmpty {
                                BLEDiagnostics.shared.recordType43IMUDecodeFailed("type-43 IMU frame had no accelerometer samples")
                            }
                        }
                    case .optical:
                        BLEDiagnostics.shared.recordType43OpticalIgnored()
                    case .unknown:
                        BLEDiagnostics.shared.recordType43UnknownVariant(length: info.dataLength)
                    }
                }
                if frameType == 40 {
                    BLEDiagnostics.shared.recordFrame40DecodeAttempt()
                }
                let route = router.handle(frame: frame)          // UI (always)
                if frameType == 40 {
                    if case .realtimeDataDecoded = route {
                        // Persisted via FrameRouter.onRealtimeHeartRate, immediately after the
                        // UI-visible `state.heartRate` assignment.
                    } else if case .realtimeDataMissingHeartRate = route {
                        BLEDiagnostics.shared.recordFrame40DecodeFailed("REALTIME_DATA without heart_rate")
                    } else {
                        BLEDiagnostics.shared.recordFrame40DecodeFailed("router result: \(route)")
                    }
                }
                if frame.count > 6, frame[6] == WhoopCommand.getDataRange.rawValue,
                   let response = BLEManager.dataRangeBounds(from: frame) {
                    strapNewestTs = response.newest               // feeds the liveness watchdog
                    BLEDiagnostics.shared.recordDataRangeResponse(oldest: response.oldest, newest: response.newest)
                }
    ...
    public func peripheral(_ peripheral: CBPeripheral,
        ...
        if characteristic.uuid == BLEManager.dataNotifyChar {
            BLEDiagnostics.shared.setDataNotifyActive(characteristic.isNotifying)
        }
    }
}
