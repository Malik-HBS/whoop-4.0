...
final class ServerSync {
    ...
    private func getStreamRows(kind: String, from: Int, to: Int, limit: Int) async -> (rows: [[String: Any]], rawCount: Int)? {
        ...
        guard let body = await get(path: path, recordDiagnostics: true).data,
    ...
    func getWorkouts(from: String, to: String) async -> [Workout] {
        ...
        guard let body = await get(path: path).data,
              let arr = (try? JSONSerialization.jsonObject(with: body)) as? [[String: Any]] else {
            return []
        }
        let int = ServerSync.int
    ...
    func getProfile() async -> Profile? {
        ...
        guard let data = await get(path: path).data else { return nil }
    ...
    private func get(path: String, recordDiagnostics: Bool = false) async -> RequestResult {
        if recordDiagnostics {
            await MainActor.run {
                ServerDiagnostics.shared.recordPullStarted(path: path)
            }
        }
        guard let url = URL(string: path, relativeTo: config.baseURL)
                     ?? URL(string: config.baseURL.absoluteString + path) else {
            let detail = "Invalid server URL for \(path)"
            if recordDiagnostics {
                await MainActor.run {
                    ServerDiagnostics.shared.recordPullFinished(path: path,
                                                                statusCode: nil,
                                                                success: false,
                                                                detail: detail)
                }
            }
            return RequestResult(data: nil, success: false, statusCode: nil, detail: detail)
        }
        ...
        do {
            ...
            guard let http = response as? HTTPURLResponse else {
                let detail = "Server pull returned a non-HTTP response"
                if recordDiagnostics {
                    await MainActor.run {
                        ServerDiagnostics.shared.recordPullFinished(path: path,
                                                                    statusCode: nil,
            ...
            let detail = success ? "GET \(path) succeeded" : "GET \(path) returned HTTP \(http.statusCode)"
            if recordDiagnostics {
                await MainActor.run {
                    ServerDiagnostics.shared.recordPullFinished(path: path,
            ...
            return RequestResult(data: success ? data : nil, success: success, statusCode: http.statusCode, detail: detail)
        } catch {
