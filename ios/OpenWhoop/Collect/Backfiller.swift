...
final class Backfiller {
    ...
    func ingest(_ frame: [UInt8]) async {
        ...
        if parsed.typeName == "HISTORICAL_DATA" {
            if let ts = parsed.parsed["unix"]?.intValue {
                BLEDiagnostics.shared.recordFrame47Decoded(timestamp: ts)
            } else {
                BLEDiagnostics.shared.recordFrame47DecodeFailed("type-47 without unix")
            }
        }

        switch classifyHistoricalMeta(parsed) {
        case .start:
            BLEDiagnostics.shared.recordHistoryMetadata(.start)
            isBackfilling = true
            chunk.removeAll(keepingCapacity: true)
            chunkOpen = true
        case .end(let unix, let trim):
            BLEDiagnostics.shared.recordHistoryMetadata(.end(unix: unix, trim: trim))
            await finishChunk(unix: unix, trim: trim, endFrame: frame)
        case .complete:
            BLEDiagnostics.shared.recordHistoryMetadata(.complete)
            isBackfilling = false
            chunk.removeAll(keepingCapacity: true)
            chunkOpen = false
        case .other:
    ...
    private func finishChunk(unix: UInt32, trim: UInt32, endFrame: [UInt8]) async {
        ...
        if !frames.isEmpty {
            ...
            do {
                ...
                BLEDiagnostics.shared.recordBackfillChunkPersisted(
                    spo2: decoded.spo2.count,
                    skinTemp: decoded.skinTemp.count,
                    resp: decoded.resp.count,
                    gravity: decoded.gravity.count
                )
            } catch {
                BLEDiagnostics.shared.recordBackfillFailure("Persistence failure: \(error.localizedDescription)")
                return
            }
            ...
            if enableRawCapture {
                ...
                do { try await store.enqueueRawBatch(meta, frames: frames) } catch {
                    BLEDiagnostics.shared.recordBackfillFailure("Raw persistence failure: \(error.localizedDescription)")
                    return
                }
            }
        }
        do { try await store.setCursor("strap_trim", Int(trim)) } catch {
            BLEDiagnostics.shared.recordBackfillFailure("Cursor persistence failure: \(error.localizedDescription)")
            return
        }
